...
FUNCTION halfRadius,r,dlnx,quantity,inner=inner
    ...
    ; simplest option:
    overall = TOTAL((2*!pi*sinh(dlnx) * r*r * quantity)) + inner
    FOR i=0, n_elements(r)-1 DO BEGIN
        IF(inner + TOTAL((2*!pi*sinh(dlnx)*r[0:i]*r[0:i]*quantity[0:i])) GT .5*overall) THEN RETURN,r[i]
    ENDFOR
...
;    IF(theBulgeExcess LE 0) THEN PRINT, "bulge excess didn't converge for name, zj : ",model.name," ",zj
    ...
    BulgeM = macc[zj] + TOTAL((2*!pi*model.Radius*model.mdotext0*(sinh(model.dlnx))*(kmperkpc/speryear)/model.vphiR) * x[*]*x[*]*(-outS[zj,*]-starS[zj,*]-gasS[zj,*]+starS[0,*]+gasS[0,*])) ;; kpc* MSol/yr *s/km * (1/speryear) * (kmperkpc) = MSol

    ;;    initial stellar mass - current stellar mass + quantity of stars ever formed = mass of stars which have migrated directly into the bulge.
    BulgeMSt = TOTAL((2*!pi*model.Radius*model.mdotext0*(sinh(model.dlnx))*(kmperkpc/speryear)/model.vphiR) * x[*]*x[*]*(-starS[zj,*] + starS[0,*] + model.Rf * model.dataCube[zj,*,51]))

    BulgeGas = BulgeM - BulgeMSt ; total amount of gas which has ever been in the bulge.

    RemnantBulgeMass = BulgeGas * model.Rf / (model.Rf+model.mlf) + BulgeMSt
    ...
    dimensionlessMbulge = RemnantBulgeMass * model.vphiR / (model.radius * model.mdotext0) * 1.0e5*speryear/(cmperkpc * Msol) ; = pi S_bulge,eff x_0^2
    sStJ = model.Radius*model.vphiR*(dimensionlessMbulge*model.dataCube[zj,0,16-1]*x[0]+TOTAL(2.0*!pi*(sinh(model.dlnx))*x*x*x*model.dataCube[zj,*,16-1]*model.dataCube[zj,*,6-1]))/(dimensionlessMbulge+TOTAL(2*!pi*(sinh(model.dlnx))*x*x*model.dataCube[zj,*,5]))
    ...
    totFg = TOTAL ( gasMass ) / (TOTAL (gasMass + stellarMass) + RemnantBulgeMass)
    ...
    fgL = TOTAL( gasMass[0:optInd] ) / (TOTAL(gasMass[0:xoutInd] + stellarMass[0:xoutInd]) + RemnantBulgeMass)
    ...
    molfg = TOTAL( gasMass* fH2 )/(TOTAL(gasMass + stellarMass)+RemnantBulgeMass)
    tdepAllGas = tdepAvg / totfH2
    stZ = .02*((TOTAL(stellarMass * 10.0^stMetallicity) + RemnantBulgeMass*10.0^bulgeMetallicity)/(stMass + RemnantBulgeMass))
    ...
    eff = (stMass+remnantBulgeMass)/(model.evArray[19-1,zj]*.18)
    BTguess = (RemnantBulgeMass + dimensionalExcess)/(stMass + RemnantBulgeMass)
    ...
    info=[xin*model.Radius,$    ;; inner edge of SF region  - 1
        ...
        RemnantBulgeMass/(stMass+RemnantBulgeMass), gasZ*.02, $ ; 50,51
        ...
        (stMass+RemnantBulgeMass)/(0.18 * eff), $ ;57
