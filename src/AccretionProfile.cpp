    ...
    if(alpha != 0.0 || profile[1]<0.0) {
        ...
        for(unsigned int n=1; n<=nx; ++n) {
            ...
            if(whichProfile == 2) { // gaussian
                ...
                double fraction = ((exp(-(currentRadialScale-xlo)*(currentRadialScale-xlo)/(2.0*sigma*sigma)) - exp(-(currentRadialScale-xhi)*(currentRadialScale-xhi)/(2.0*sigma*sigma)))*sqrt(2.0*M_PI)*sigma + currentRadialScale*M_PI*(erf((currentRadialScale-xlo)/(sigma*sqrt(2.0))) - erf((currentRadialScale-xhi)/(sigma*sqrt(2.0))))) / normalization;
            ...
            if(profile[n]<0.0) errormsg("Negative accretion profile! This could mean you've selected an invalid/unsupported accretion profile");
        }
    }
...
double AccretionProfile::fInner()
{
    ...
    if(whichProfile == 2)
        fracInner = (exp(-currentRadialScale*currentRadialScale / (2.0*sigma*sigma)) - exp(-(currentRadialScale - xinner)*(currentRadialScale - xinner)/(2.0*sigma*sigma))) * sqrt(2.0*M_PI)*sigma + currentRadialScale*M_PI*(erf(currentRadialScale/(sqrt(2.0)*sigma)) - erf((currentRadialScale - xinner)/(sqrt(2.0)*sigma)));

   if(fracInner > 1.0 || fracInner < 0.0) errormsg("A nonphysical fraction of the accretion has occurred interior to the innermost radius."); 
   return fracInner;
}
